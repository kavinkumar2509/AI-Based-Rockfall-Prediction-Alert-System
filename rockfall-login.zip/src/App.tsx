...
export default function App() {
  ...
  const [operatorId, setOperatorId] = useState<string>("RF-1984-X");
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (!password) {
      ...
      setStatusMessage("ERROR: AUTHENTICATION PROTOCOL KEY IS BLANK");
      return;
    }

    setIsConnecting(true);
    setConnectionStatus("AUTHENTICATING");
    setStatusMessage("AUTHENTICATING: DECRYPTING SIGNATURE...");

    setTimeout(() => {
      setIsConnecting(false);
      setConnectionStatus("CONNECTED");
      setStatusMessage(`LINK VERIFIED: WELCOME OPERATOR ${operatorId}`);
      
      // Auto-revert back to idle after 5 seconds
      setTimeout(() => {
        setConnectionStatus("IDLE");
        setStatusMessage("SYSTEM STATUS: CHANNELS SECURED");
      }, 5000);
    }, 1500);
  };
